...
songDF = pd.read_csv("python/data/allsong_data.csv")
complete_feature_set = pd.read_csv("python/data/complete_feature.csv")
playlistDF_test = pd.read_csv("python/data/test_playlist.csv")
...
def recommend(playlist):
    #only playlist name stored in data file
    playlist_path=os.path.join('python/data',playlist)
    playlistDF_test = pd.read_csv(playlist_path)
    print(recommend_from_playlist(playlistDF_test=playlistDF_test)[:10])
# if __name__ == '__main__':
#     print(recommend_from_playlist()[:10])

recommend('test_playlist.csv')
#replace test_playlist.csv with the csv file of the user history